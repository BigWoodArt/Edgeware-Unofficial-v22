...
class Popup(Toplevel):
    ...
    def __init__(self, root: Tk, settings: Settings, pack: Pack, state: State, on_close: Callable[[], None] | None = None) -> None:
        ...
        # On Windows, a freshly spawned process's first topmost window can
        # still end up behind whatever already has focus - Windows' focus-
        # stealing prevention doesn't treat "-topmost" alone as a request for
        # focus. This bites hardest right when Edgeware is launched from
        # config.pyw's "Save, Exit, and Run", since the config window is
        # closing at the same moment the new process's first popup appears.
        # Toggling topmost off and back on once the window is mapped, plus
        # lift()/focus_force(), is the standard reliable fix for this.
        self.after(50, self._reassert_topmost)
        set_borderless(self)
    ...
    def _reassert_topmost(self) -> None:
        try:
            ...
            pass  # Popup was already closed before this ran
